...
# File paths
esslnc_path = f'../../data/benchmark/{args.species}/ess_lnc.csv'
nonesslnc_path = f'../../data/benchmark/{args.species}/noness_lnc.csv'
...
# Combine datasets
y_all = np.hstack((np.ones(len(esslnc_id)), np.zeros(len(nonesslnc_id))))
ids_all = np.hstack((list(esslnc_id), list(nonesslnc_id)))

# Create directory to store shuffled datasets
output_dir = f'./{args.species}_shuffled_new'
...
for shuffle_num in range(1000):
    ...
    shuffled_file_path = os.path.join(output_dir, f'shuffled_{shuffle_num+1}_new.csv')
